...
async def cb_admin_list(callback: CallbackQuery, db: DatabaseService) -> None:
    ...
    await safe_edit(callback.message, text, reply_markup=kb_group_list(groups, active_filter))
    await callback.answer()
...
async def cb_admin_cleanup_dupes(callback: CallbackQuery, db: DatabaseService) -> None:
    ...
    deleted = 0
    details = []
    for _keep, remove in dupes:
        details.append(
            f"🗑 <b>{remove.full_name}</b> ({remove.group_name}) — Lv.{remove.level} {remove.xp} XP o'chirildi"
        )
...
async def cb_delete_all_msgs(callback: CallbackQuery, bot: Bot, db: DatabaseService) -> None:
    ...
    await safe_edit(callback.message, f"⏳ <b>{len(groups)} ta guruhdan xabar o'chirilmoqda...</b>")
    await callback.answer()
    ...
    result_text = f"🗑 <b>Xabarlar o'chirildi!</b>\n\n✅ Muvaffaqiyatli: {ok} ta\n"
    if already:
...
async def fsm_got_name(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(
        f"✅ Nom: <b>{name}</b>\n\n<b>3/4 — Jadval turi</b>\n\nBu guruh qaysi kunlarda dars oladi?",
...
async def fsm_got_type(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await safe_edit(
        callback.message,
        f"✅ Jadval: <b>{label}</b>\n\n<b>4/4 — Auditoriya turi</b>\n\nBu guruh kimlar uchun?",
...
async def fsm_got_audience(callback: CallbackQuery, state: FSMContext, db: DatabaseService) -> None:
    raw = callback.data.split(":")[2]
    ...
    try:
        ...
        logger.info(f"Yangi guruh: '{name}' | {group_type.value} | {audience.value}")
    except Exception as e:
...
async def cb_select_toggle(callback: CallbackQuery, state: FSMContext, db: DatabaseService) -> None:
    ...
    bot_chats = await db.get_bot_chats()
    await safe_edit_markup(callback.message, reply_markup=kb_group_selector(bot_chats, selected))
    await callback.answer()
...
async def cb_select_none(callback: CallbackQuery, state: FSMContext, db: DatabaseService) -> None:
    ...
    bot_chats = await db.get_bot_chats()
    await state.update_data(selected=[])
    await safe_edit_markup(callback.message, reply_markup=kb_group_selector(bot_chats, set()))
...
async def cb_select_confirm(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await safe_edit(
        callback.message,
        f"✅ <b>{len(selected)} ta guruh tanlandi</b>\n\n<b>Jadval turi</b>\n\nBu guruhlar qaysi kunlarda dars oladi?",
        reply_markup=kb_choose_type(),
    )
    await callback.answer()
...
async def bulk_got_type(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await safe_edit(
        callback.message,
        f"✅ Jadval: <b>{label}</b>\n\n<b>Auditoriya</b>\n\nBu <b>{count} ta</b> guruh kimlar uchun?",
        reply_markup=kb_choose_audience(),
    )
    await callback.answer()
...
async def bulk_got_audience(callback: CallbackQuery, state: FSMContext, db: DatabaseService) -> None:
