...
def kb_admin_students(
    ...
    filtered = (
        students if active_group == "all"
        else [s for s in students if s.group_name == active_group]
    )

    if not filtered:
        builder.row(InlineKeyboardButton(
            text="📭 Hali hech kim ro'yxatdan o'tmagan", callback_data="noop"
        ))
    else:
        for s in filtered:
            label = f"👤 {s.full_name}"
...
def kb_group_list(groups: list[Group], active_filter: str = "all") -> InlineKeyboardMarkup:
    ...
    if not filtered:
        builder.row(
            InlineKeyboardButton(text="📭 Guruhlar yo'q", callback_data="noop")
        )
    else:
        for g in filtered:
...
def kb_group_actions(group: Group) -> InlineKeyboardMarkup:
    ...
    if group.is_active:
        toggle_btn = InlineKeyboardButton(
            text="⏸ Nofaol", callback_data=f"group:off:{group.chat_id}"
        )
    else:
        toggle_btn = InlineKeyboardButton(
            text="▶️ Aktiv", callback_data=f"group:on:{group.chat_id}"
        )
...
def kb_curator_students(
    ...
    for g in all_groups:
        cnt  = sum(1 for s in students if s.group_name == g)
        mark = "●" if active_group == g else "○"
        grp_btns.append(InlineKeyboardButton(
            text=f"{mark} {g} ({cnt})",
