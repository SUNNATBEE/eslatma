...
def setup_student_routes(app: web.Application, ctx: dict) -> None:
    ...
    async def api_homework(request: web.Request) -> web.Response:
        ...
        if not student:
            return json_err("Not registered", code="not_registered", status=404)
        # Homework keyingi darsgacha amal qiladi — dars boshlanganda
        # scheduler avtomatik o'chiradi (expire_homeworks_at_class_start).
    ...
    async def api_hw_history(request: web.Request) -> web.Response:
        ...
        return web.json_response(
            {"items": [{"sent_at": h.sent_at.strftime("%d.%m.%Y %H:%M"), "message_id": h.message_id} for h in history]}
        )
    ...
    async def api_student_register(request: web.Request) -> web.Response:
        ...
        if not (mars_id.isdigit() or (mars_id.startswith("P") and mars_id[1:].isdigit())):
            return json_err(
                "Mars ID faqat raqam yoki P12345 formatida bo'lishi kerak",
                code="invalid_mars_id",
                status=400,
            )
        if not re.fullmatch(r"\+998\d{9}", phone):
            return json_err("Telefon formati: +998901234567", code="invalid_phone", status=400)
        ...
        if not cred:
            return json_err("Bu Mars ID topilmadi", code="unknown_mars_id", status=403)
        if not verify_secret(cred["password"], password):
            return json_err("Parol noto'g'ri", code="invalid_password", status=403)
        ...
        if cred["group"] != group_name:
            return json_err(
                f"Sizning guruhingiz: {cred['group']}. {group_name} ni tanlamang.",
                code="wrong_group",
                status=403,
            )
        ...
        if existing and existing.user_id != user_id:
            return json_err(
                "Bu Mars ID boshqa Telegram akkountda ro'yxatdan o'tilgan. Admin bilan bog'laning.",
                code="mars_id_taken",
                status=409,
            )
        ...
        notif_header = (
            "🔔 <b>Yangi o'quvchi (Mini App)</b>" if is_new else "🔄 <b>Mavjud o'quvchi qayta kirdi (Mini App)</b>"
        )
        notify = (
            f"{notif_header}\n\n"
            f"👤 {cred['name']}\n"
    ...
    async def api_student_mood(request: web.Request) -> web.Response:
        ...
        if request.method == "POST":
            ...
            if mood not in ("happy", "ok", "sad"):
                return json_err("Invalid mood", code="invalid_mood", status=400)
    ...
    async def api_student_profile(request: web.Request) -> web.Response:
        ...
        try:
            ...
            return json_err("Invalid user_id", code="invalid_user_id", status=400)
    ...
    async def api_student_logout(request: web.Request) -> web.Response:
        ...
        notif = f"🚪 <b>O'quvchi akkountdan chiqdi</b>\n\n👤 {name}\n📚 Guruh: {group}\n🆔 TG: <code>{user_id}</code>"
        for admin_id in ADMIN_IDS:
            try:
    ...
    async def api_student_avatar(request: web.Request) -> web.Response:
        ...
        if not avatar:
            return json_err("Avatar bo'sh bo'lmasin", code="validation_error", status=400)
    ...
    async def api_chat_post(request: web.Request) -> web.Response:
        ...
        if not text:
            return json_err("Xabar bo'sh bo'lmasin", code="validation_error", status=400)
        if len(text) > 500:
            return json_err("Xabar 500 belgidan oshmasin", code="validation_error", status=400)
    ...
    async def api_student_pending_register(request: web.Request) -> web.Response:
        ...
        if has_group and not all([group_time, group_day_type, teacher_name]):
            return json_err("Guruh ma'lumotlarini to'liq kiriting", code="validation_error", status=400)
        try:
